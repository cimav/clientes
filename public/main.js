...
var camera,
    ...
    ratio = width / height;
var texture = THREE.ImageUtils.loadTexture('suzuki-pano.jpg', new THREE.UVMapping(), function() {
    init();
    animate();
});
function init() {
...
function render() {
    ...
    lat = 25.00; //Math.max(-85, Math.min(85, lat));
    lon = Math.max(-15, Math.min(15, lon));
    if (lon >=15) inc = -0.05;
    if (lon <=-15) inc = 0.05;
    phi = THREE.Math.degToRad(90 - lat);
    theta = THREE.Math.degToRad(lon);
    var x = 100 * Math.sin(phi) * Math.cos(theta);
    var y = 100 * Math.cos(phi);
    var z = 100 * Math.sin(phi) * Math.sin(theta);
    //z = z < -80.00 ? 80.00 : z > 90.00 ? 90.00 : z;
    camera.position.x = x;
    camera.position.y = y;
    camera.position.z = z;
